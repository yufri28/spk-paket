...
// Aktifkan buffer output
ob_start();

// Fungsi untuk menangkap kesalahan
function errorHandler($errno, $errstr, $errfile, $errline) {
    // // Log kesalahan jika perlu atau langsung redirect
    // header('Location: ./index.php');
    // exit;
    echo "<script>alert('Terjadi kesalahan. Data alternatif belum lengkap. Silahkan lengkapi data alternatif terlebih dahulu!');
    window.location.href = './index.php';
    </script>";
    exit;
}

// Set error handler custom
set_error_handler('errorHandler');
  ...
  for($i=0; $i < $n_subject; $i++){
    ...
    while ($row = $data->fetch_object()) {
      ...
      echo "<script>alert('Terjadi kesalahan. Data alternatif belum lengkap. Silahkan lengkapi!');
        window.location.href = './index.php';
      </script>";
      exit;
  }
// Akhir script, flush output buffer
ob_end_flush();
?>
