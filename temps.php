...
if (isset($_POST['hasil-perengkingan'])) {
  ...
  try {
    ...
    $value = array();
    $sql = "SELECT a.id_alternatif, b.id_kriteria, kac.detail,
    ...
    $get = $koneksi->query("SELECT kac.*,sk.*, k.*, a.konsep_gedung, sk.bobot_sub_kriteria AS nilai 
              FROM kec_alt_kriteria kac 
              JOIN sub_kriteria sk ON sk.id_sub_kriteria=kac.f_id_sub_kriteria 
              JOIN alternatif a ON a.id_alternatif=kac.f_id_alternatif 
              JOIN kriteria k ON kac.f_id_kriteria=k.id_kriteria
...
foreach ($get as $value) {
    ...
    $f_id_kriteria = $value['f_id_kriteria'];
    $detail = $value['detail'];

    // Membuat array dengan struktur yang lebih rapi
    $det_kri[$f_id_alternatif]['detail-' . $f_id_kriteria] = $detail;
    $det_kri[$f_id_alternatif]['type'] = $value['jenis_kriteria'];
}

// Fungsi pembanding untuk pengurutan
usort($det_kri, function ($a, $b) {
  // Konversi nilai yang kosong menjadi 0 untuk memastikan bisa diurutkan
  $aC1 = !empty($a['detail-C1']) ? $a['detail-C1'] : 0;
  $bC1 = !empty($b['detail-C1']) ? $b['detail-C1'] : 0;

  // Urutkan berdasarkan type
  if ($a['type'] === 'Benefit' && $b['type'] === 'Cost') {
      return -1; // Benefit lebih baik dari Cost
  }
  if ($a['type'] === 'Cost' && $b['type'] === 'Benefit') {
      return 1; // Cost kurang baik dari Benefit
  }
  
  // Jika jenis sama, lakukan pengurutan sesuai jenisnya
  if ($a['type'] === 'Benefit') {
      // Jika Benefit, urutkan descending
      return $bC1 <=> $aC1;
  } else {
      // Jika Cost, urutkan ascending
      return $aC1 <=> $bC1;
  }
});
